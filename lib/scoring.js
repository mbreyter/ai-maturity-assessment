...
export function calculateScores(answers) {
  ...
  const maturityStage = determineStage(stageScores);
  const stageNames = {
    1: "The Petting Zoo",
    2: "The Bazaar",
  ...
  // Each dimension has one Stage 1 question (Q1-Q5) so early-stage companies
  // show non-zero scores; Measurement correctly shows 0 until outcomes are tracked.
  const dimensionQuestions = {
    "Strategy":            [5, 9, 16, 17, 26],
    "Governance":          [3, 10, 14, 18, 23],
    "Operating Model":     [2, 12, 13, 27, 30],
    "Workforce":           [4, 7, 19, 20, 29],
    "Data & Technology":   [1, 6, 8, 21, 24],
    "Measurement & Value": [11, 15, 22, 25, 28],
  };
  // Q1-Q5: score 2 = more mature (use directly)
  // Q6-Q30: score 2 = less mature (invert so 2→0, 0→2)
  const directScoreQuestions = new Set([1, 2, 3, 4, 5]);
  const dimensionScores = {};
  for (const [dim, qIds] of Object.entries(dimensionQuestions)) {
    dimensionScores[dim] = qIds.reduce((sum, id) => {
      const raw = parseInt(answers[id]) || 0;
      return sum + (directScoreQuestions.has(id) ? raw : 2 - raw);
    }, 0);
  }
...
function determineStage(stageScores) {
  // Q1-Q5 (Stage 1): score 2 = most mature → mastery = stageScores[1] directly
  // Q6-Q30 (Stages 2-6): score 0 = most mature → mastery = 10 - stageScores[i]
  // Total mastery ranges 0-60; each 10 points = one stage of progress
  const stageMastery = [
    stageScores[1],
    10 - stageScores[2],
    10 - stageScores[3],
  ...
  const totalMastery = stageMastery.reduce((a, b) => a + b, 0);
  return Math.min(6, Math.max(1, Math.floor(totalMastery / 10) + 1));
}
